...
class CycleGAN(keras.Model):
    def __init__(self, p_lambda=LAMBDA, r_loss_factor=R_LOSS_FACTOR, **kwargs):
        ...
        # VAE Model
        self.vae = VAE(r_loss_factor=r_loss_factor, summary=False)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            # VAE Operations
            vae_x = self.vae.encoder_model(real_x)
            vae_z, z_mean, z_log_var = self.vae.sampler_model(vae_x)
            vae_y = self.vae.decoder_model(vae_z)

            # VAE Loss
            r_loss = self.vae.r_loss_factor * self.vae.mae(real_y, vae_y)
            kl_loss = -0.5 * (1 + z_log_var - tf.square(z_mean) - tf.exp(z_log_var))
            kl_loss = tf.reduce_mean(tf.reduce_sum(kl_loss, axis=1))
            total_loss = r_loss + kl_loss
        ...
        # VAE Gradients
        vae_gradients = tape.gradient(total_loss, self.vae.trainable_weights)

        # VAE optimizer step
        self.vae_optimizer.apply_gradients(zip(vae_gradients,
        ...
        return {
            "vae_loss": self.total_loss_tracker.result(),
            "vae_reconstruction_loss": self.reconstruction_loss_tracker.result(),
            "vae_kl_loss": self.kl_loss_tracker.result(),
